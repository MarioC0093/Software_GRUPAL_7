...
```
Se crea un dataset con información a nivel municipio con el identificador de este y datos de censo, mesas y participación para cada fecha.
...
En ls distribución de las variables numéricas concretamos algunas variables puede ser interesante obseravarlas con su trasformación logarítma debido a su gran dispersión y otras (los cálculos de porcentajes) parecen tener valores incoherentes (mayor número de votos que censados)

```{r}
#| collapse: true
skim(datos_elecciones_maestra)

datos_elecciones_maestra |> select(is.numeric) |> 
...
```
Eliminamos registros donde el número de votos es superior al número de censados.

```{r}
demasiados_censados <- datos_elecciones_maestra |> filter(p_votos>1)

datos_elecciones_maestra <- datos_elecciones_maestra |> anti_join(demasiados_censados, by=c("fecha","cod_mun") )
```

También se encuentran datos de participación en el primer y segundo avance que parecen ser erróneos: 
-   Datos del primer avance superiores al segundo avances.
-   Datos del segundo avance.
-   Participaciones mayores a 1.

Los registros con el segundo avance a cero cuando en el primer avance sí ha habido información vamos a imputarlos. Los casos en los que tanto el primer como el segundo avance es cero, se dan por válidos ya que el censo de esos municipios es muy pequeño (vamos a suponer que todos los censados del pueblo fueron a votar a última hora).
...
::: callout-note
imputación de los datos para participación_2==0 cuando participacion_1!=0
:::

En los casos donde los datos del primer avance son superiores al segundo avance se busca municipios con un censo de más/menos diez por ciento y se sustituye el dato de participación de estos registros por la media de los municipios buscados, ya que son los municipios más homogéneos a él.


```{r}
datos_elecciones_maestra |> filter( p_participacion_1>p_participacion_2 ) |> select(fecha,cod_mun,censo,p_participacion_1,p_participacion_2, p_votos)
```
...
correcion_df <- function(data, cond_errata){
  a <- data |> #select(fecha,cod_mun,censo,p_participacion_1,p_participacion_2) |> 
    filter( eval(parse(text=cond_errata)) )
...
correcion_dato_censal <- function(data, var_errata, fecha_errata, mun_errata){
  ...
  varlor_new <- data |> filter(fecha==fecha_errata & cod_mun!=mun_errata & eval(parse(text=var_errata))<=1) |>
    filter(between(censo,
                   censo_errata-censo_errata*0.10,
                   censo_errata+censo_errata*0.10)) |> select(var_errata) |> sapply(, FUN="mean")
...
```{r}
datos_elecciones_maestra2 <- datos_elecciones_maestra |> anti_join(new_data, by=c("fecha","cod_mun"))
datos_elecciones_maestra3 <- rbind(datos_elecciones_maestra2,new_data)
```

```{r}
datos_elecciones_maestra3 |> filter( p_participacion_1>1 | p_participacion_1>p_participacion_2 ) |> select(fecha,cod_mun,censo,p_participacion_1,p_participacion_2)
```


```{r}
#| collapse: true
skimr::skim(datos_elecciones_maestra,)
...
siglas_aut_y_nac <- function(data){
  data <- data |> mutate(siglas2 = if_else(siglas %in% c("PSOE","PP","VOX","CS","MP","UP"),
...
Ademas siempre están las apuestas de "si la participación es alta beneficia a la derecha o a la izquierda", etc.
...
# como pte
datos_participacion_fecha_municipio <- datos_elecciones |> distinct(fecha, cod_mun, censo, participacion_1, participacion_2, votos_blancos, votos_nulos, votos_candidaturas) |> 
...
datos_participacion_municipio <- datos_elecciones |> distinct(fecha, cod_mun, censo, participacion_1, participacion_2, votos_blancos, votos_nulos, votos_candidaturas) |> 
  mutate(votos=votos_blancos+votos_nulos+votos_candidaturas) |>
  left_join(mas_votado1 |> select(fecha,cod_mun,siglas_ganador=siglas)) |>
  summarise(censo=sum(censo),
            votos=sum(votos),
...
```{r}
#| eval: false
# no me puto sale. y lo más parecido que sale no da los resultados que quería
...
::: callout-warning
Añadir comentarios al segundo chunk (chunk_comentar) sobre que el cambio de votos en cada provincia hacia los partidos a medida que aumenta el censo. Valorar si se hace por siglas o por siglas2. PTE de verlo con colores para ver cómo queda.

No se me ocurre otra forma de hacerlo.
:::

```{r}
datos_porcentaje_voto |> select(fecha, cod_mun, codigo_ccaa, censo, votos_candidaturas, p_voto) |> mutate(censo_quantile = ntile(censo , 20)) |> left_join(mas_votado1 |> select(fecha, cod_mun,siglas)) |> mutate(siglas2 = if_else(siglas %in% c("PSOE","PP","VOX","CS","MP","UP"),
                                           "part. principales", "part. aut./nac.")) |> 
  ggplot()+
