...
La práctica versará entorno a los **archivos de datos electorales** que ya hemos trabajado, recopilando datos de citas electorales al Congreso de los Diputados de España desde 2.008 hasta la actualidad. Son cuatro las bases de datos de las que se parten.
...
La base de datos datos_elecciones original es un tibble de 48.737 x 471 lo que impide su comprensión para un correcto análisis de datos.

```{r}
datos_elecciones |> head()
```

Es preferible una columna con todos los partidos políticos y otra con el número de votos de cada uno en cada municipio en cada una de las elecciones registradas. Se eliminan los valores NA, ya que estos registros hacen referencia a partidos que no han recibido votos en esas elecciones. También se eliminan aquellas columnas que no aportan información, es decir, todas aquellas que tengan un único valor.
...
Se obtiene como resultado un tibble de 396.735 x 14.
...
Se crea una nueva columna fecha (columnas anno y mes) y una nueva id_mun que funciona como id para unir las bases de datos datos_elecciones y cod_mun. Se eliminan las columnas *anno*, *mes*, *codigo_provincia* y *codigo_municipio"* dado que se han creado nuevas variables con estos datos pero se mantiene la columna *codigo_ccaa*.
...
Unimos las tablas datos_elecciones y cod_mun a fin de que la base final tenga el nombre de los municipios.
...
Se realiza una normalización de la base de datos siglas y a continuación se une a la tabla datos_elecciones.
...
datos_elecciones |> count(fecha, codigo_ccaa) |>
  ...
  slice_max(fecha) |> head(10) |> rename(`Freq muestra(n)`=n,"Freq muestra(%)"=p)
```
...
Verificamos cuál de las dos formas es más rápida en nuestra muestra aleatoria estratificada.

```{r microbenchmark-show}
#| echo: true
#| eval: false
...
Agrupamos los datos usando summarise. (sustituir datos_elecciones_new por datos_elecciones)

```{r}
#| echo: true
datos_elecciones_new <- datos_elecciones |>
  summarise(votos = sum(votos), .by=everything()[! everything() %in% c('votos')])
```

```{r}
datos_elecciones_new |> head(10)
```
...
correcion_dato_censal <- function(data, var_errata, fecha_errata, mun_errata){
  ...
  varlor_new <- data |> filter(fecha==fecha_errata & cod_mun!=mun_errata & eval(parse(text=var_errata))<=1 & p_participacion_2<=part_2_tope) |>
    filter(between(censo,
                   ...
                   censo_errata+censo_errata*0.10)) |> select(all_of(var_errata)) |> sapply(, FUN="mean", na.rm = TRUE)
