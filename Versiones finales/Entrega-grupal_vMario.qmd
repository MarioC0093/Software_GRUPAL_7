...
Distribución de votos para los registros con censo mayor a 2.000.

```{r}
#| results: hold
data_2 <- mas_votado |> anti_join(datos_elecciones |> select(fecha, cod_mun, codigo_ccaa, censo) |> filter(censo<2000),
                              by = c("fecha", "cod_mun")) |> 
...
```{r}
findoutlier <- function(x,k=3) {
  return(x < quantile(x, .25) - k*IQR(x) | x > quantile(x, .75) +k*IQR(x))
}
...
data_2 |> filter(lag_votos>0) |> 
  ...
  mutate(outlier = ifelse(findoutlier(lag_votos,k=3), lag_votos, NA)) |> 
  ggplot(aes(y = log(lag_votos) , x=codigo_ccaa))+
  geom_boxplot()+
...
data_2 |> filter(lag_votos>0) |> 
  group_by(fecha, codigo_ccaa) |> 
  mutate(lag_votos_outlier_mad = if_else(abs(scores(lag_votos, type = "mad")) > k_mad, 1, NA),
         lag_votos_outlier_iqr = if_else(abs(scores(lag_votos, type = "iqr")) > k_iqr, 1, NA)) |> 
  ggplot(aes(y = log(lag_votos) , x=codigo_ccaa))+
  geom_boxplot()+
  geom_text(aes(label=lag_votos_outlier_iqr), na.rm=TRUE, hjust=-.5)+
  facet_wrap(~fecha)
...
data_2 |> filter(lag_votos>0) |> 
  group_by(fecha, codigo_ccaa) |> 
  mutate(lag_votos_outlier_mad = if_else(abs(scores(lag_votos, type = "mad")) > k_mad, 1, NA),
         lag_votos_outlier_iqr = if_else(abs(scores(log(lag_votos), type = "iqr")) > log(k_iqr), 1, NA)) |> 
  ggplot(aes(y = log(lag_votos) , x=codigo_ccaa))+
  geom_boxplot()+
  geom_text(aes(label=lag_votos_outlier_iqr), na.rm=TRUE, hjust=-.5)+
  facet_wrap(~fecha)
...
data_2 |> filter(lag_votos>0) |> 
  group_by(fecha, codigo_ccaa) |> 
  mutate(outlier = if_else(abs(scores(lag_votos, type = "mad")) > k_mad, 1, NA),
         lag_votos_outlier_iqr = if_else(abs(scores(log(lag_votos), type = "iqr")) > log(k_iqr), 1, NA)) |> 
  ggplot(aes(y = log(lag_votos) , x=codigo_ccaa))+
  geom_boxplot()+
  geom_text(aes(label=lag_votos_outlier_iqr), na.rm=TRUE, hjust=-.5)+
