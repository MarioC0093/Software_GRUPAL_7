...
--> ToDo: hacer demostración de summary(datos$votos) para ver que el mínimo es 1 y confirmar que NA es que no ha recibido votos, y que no hay distinción entre NA y 0.
...
Se procede a añadir las siglas de los partidos a la tabla de datos_encuestas (que incluye información de cod_mun).
Nota de Mario: datos_encuestas no sé de dónde sale,
nota de Mario: by=denominacion no sé cómo es, denominación solo está en una tabla
nota de mario: si haces un inner join te quedas sin los datos.
...
!!!!! EXPLICAR TODO EL POR QUÉ NO NOS SIRVE SIGLAS (lo que hablamos en la reu) --> MARIO
...
encuestas %>% select(all_of(cols_sin_informacion)) %>% summary() # todo NA excepto exit_poll que es todo a no pie de urna
...
#| eval: false
# no ejecutar
encuestas2 <- datos_elecciones |> 
  left_join(encuestas, by = "siglas")
```
