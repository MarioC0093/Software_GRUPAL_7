...
datos_elecciones <- datos_elecciones |>
  mutate(fecha=glue("{anno}-{mes}"),
         ...
         .before = "codigo_ccaa") |> 
  select(-c(anno, mes, codigo_provincia, codigo_municipio))
```
...
Una vez que hemos normalizado los nombre de los partidos políticos a una lista de trece siglas, agrupamos la información que antes estaba a nivel de partido político a nivel de siglas.
...
Igual que hemos visto casos donde primer y segundo puesto estaban empatados, merece la pena mirar dónde esa diferencia es máxima. Mas solo interesa aquellos con un censo alto (superior o igual a 2.000 censados) ya que en municipios más pequeños las diferencias serán muy pequeñas y nos podría distorsionar la distribución por la cola inferior.

Solo interesa municipios donde el censo sea alto, pongamos un marco de 2.000 censados en adelante. Ya que en municipios en pocos censados las diferencias entre primer y segundo partido más votado serán muy pequeñas y esto nos puede dar una distribución de la variable "difenrecia entre primer y segundo partido más votado" con una cola izquierda muy pesada.

Con esta restricción de municipios con mayor censo sabemos que si vemos una diferencia de pocos votos es realmente un resultado ajustado, ya que una diferencia en cinco o diez votos en zonas con pocos censados no tiene tanto impacto.

<!-- Conscientemente trabajamos con medidas repetidas para un mismo municipio (un dato para cada una de las elecciones). -->

Para este punto se plante agrupar los datos de los municipios ya que hasta ahora hemos estado trabajando con medidas repetidas para un mismo municipio (un dato para cada una de las elecciones).
...
```{r}
datos_elecciones |> mutate(censo_menor2000 = if_else(censo<2000,1,0)) |> distinct(fecha, cod_mun, censo_menor2000) |> pivot_wider(names_from = fecha, values_from = censo_menor2000) |> select(-cod_mun) |> group_by_all() |> summarise(n=n()) #summarise(media=mean(censo_menor2000), .by=(fecha))
```
...
```{r}
datos_elecciones |> mutate(censo_menor2000 = if_else(censo<2000,1,0)) |> distinct(fecha, cod_mun, censo_menor2000) |> pivot_wider(names_from = fecha, values_from = censo_menor2000) |> filter(`2008-03`==0) |> filter(`2011-11`==0) |> filter(is.na(`2015-12`))

datos_elecciones |> filter(cod_mun=="11-15-026") |> distinct(fecha,cod_mun,censo,votos_candidaturas)
```
...
[![Elephant](https://raw.githubusercontent.com/MarioC0093/Software_GRUPAL_7/main/desarrollo_Mario/censo_Cesuras.JPG)]


```{r}
datos_elecciones |> filter(cod_mun=="11-15-063") |> distinct(fecha,cod_mun,censo,votos_candidaturas) # Oza dos Ríos

datos_elecciones |> filter(cod_mun=="11-15-902") |> distinct(fecha,cod_mun,censo,votos_candidaturas) # Oza-Cesuras
```
...
library(skimr)
# datos_elecciones %>%
#   select(censo, codigo_ccaa, fecha) |> 
#   group_by(codigo_ccaa, fecha) %>%
#   skim()
```


```{r}
mas_votado |> anti_join(datos_elecciones |> select(fecha, cod_mun, censo) |> filter(censo<20000),
                              by = c("fecha", "cod_mun")) %>%
  group_by(fecha,cod_mun) %>% mutate(lag_votos = votos - lag(votos)) |> mutate(lag_votos=-lag_votos) |> head()
```

Distribución de votos para esos municipios.

```{r}
#| results: hold
data_2 <- mas_votado |> anti_join(datos_elecciones |> select(fecha, cod_mun, codigo_ccaa, censo) |> filter(censo<20000),
                              by = c("fecha", "cod_mun")) %>%
  group_by(fecha,cod_mun) %>% mutate(lag_votos = votos - lag(votos)) |> mutate(lag_votos=-lag_votos) |> drop_na() |> ungroup()
...
data_2 |> 
  ggplot(aes(y=log(votos)))+
  geom_boxplot()
```
Yo no mostraría estos dos gráficos:
```{r}
data_2 |> 
  ggplot(aes(x=(votos), y = codigo_ccaa , fill = codigo_ccaa))+
  geom_density_ridges()

data_2 |> 
  ggplot(aes(y = votos , x=codigo_ccaa))+
  geom_boxplot()
```


Distribución de la diferencia de votos para esos municipios.
...
La distribución del logaritmo de la difenrencia de votos sigue una distribución normal y mucho más visual que la distribucion original que nos permite la búsqueda de outliers.

```{r}
# data_2 |> 
#   ggplot(aes(x=(lag_votos), y = codigo_ccaa , fill = codigo_ccaa))+
#   geom_density_ridges()
# 
# data_2 |> 
#   ggplot(aes(y = lag_votos , x=codigo_ccaa))+
#   geom_boxplot()

data_2 |> filter(lag_votos>0) |> 
...
```{r}
data_2 |> filter(lag_votos>0) |> 
  # mutate(lag_votos_outlier_iqr = if_else(abs(scores(lag_votos, type = "iqr")) > k_iqr, 1, 0)) |> 
  mutate(outlier = ifelse(findoutlier(lag_votos), lag_votos, NA)) |> 
  ggplot(aes(y = log(lag_votos) , x=codigo_ccaa))+
  geom_boxplot()+
  geom_text(aes(label=outlier), na.rm=TRUE, hjust=-.5)
```

Búsqueda de outliers en la diferencia de votos.

```{r}
data_2 |> filter(lag_votos>0) |> 
  ggplot(aes(y = log(lag_votos) ,  colour = fecha))+
  geom_boxplot()
```
...
```{r}
data_2 |> filter(lag_votos>0) |> 
  ggplot(aes(y = log(lag_votos) , x=codigo_ccaa , fill = fecha))+
  geom_boxplot()
```
...
k_mad <- 5
k_iqr <- 5
data_2_outliers_mean <-
  data_2 |>
  drop_na() |> 
  mutate(lag_votos_outlier_mad = if_else(abs(scores(lag_votos, type = "mad")) > k_mad, 1, 0),
         lag_votos_outlier_iqr = if_else(abs(scores(lag_votos, type = "iqr")) > k_iqr, 1, 0))

data_2_outliers_mean |> filter(lag_votos_outlier_mad==1) |> select(cod_mun) |> table() |> sort()
data_2_outliers_mean |> filter(lag_votos_outlier_iqr==1) |> select(cod_mun) |> table() |> sort()

rm(data_2)
```

¿Tendrá algo que ver el porcentaje de voto con los resultados electorales? Miramos primero aqullso municipios y provincias con mayor porcentaje de voto.

```{r}
datos_porcentaje_voto <- datos_elecciones |> distinct(fecha,cod_mun,codigo_ccaa,censo,votos_blancos,votos_nulos,votos_candidaturas) |>
